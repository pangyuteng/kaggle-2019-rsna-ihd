...
mlp_epochs = 200
mlp_batch_size = 64
mlp_steps_per_epoch = 2000
mlp_validation_steps = 100
#************************************************************
...
mykey = user_secrets.get_secret("mykey")
myname = user_secrets.get_secret("myname")
os.makedirs('/tmp/.kaggle',exist_ok=True)
with open('/tmp/.kaggle/kaggle.json','w') as f:
    mydict = {'username':myname,'key':mykey}
...
optimizer = Adam(learning_rate=vae_learning_rate,)
...
mlp = Sequential()
mlp.add(Dense(32,input_shape=(latent_dim,)))
...
mlp.add(BatchNormalization())
mlp.add(LeakyReLU())
mlp.add(Dense(6, activation='softmax')) # sigmoid
mlp.summary()

mlp_opt = Adam(learning_rate=mlp_learning_rate) # RMSprop
mlp.compile(loss='categorical_crossentropy',# 'binary_crossentropy', # ?? categorical_crossentropy',
              optimizer=mlp_opt,
              metrics=['accuracy'])
...
try:
    ...
    validation_dg = DataGenerator(validation_file_path_list)
    ...
    train_ldg = LatentDataGenerator(encoder,ydf,train_file_path_list,latent_dim=latent_dim,enc_batch_size=mlp_batch_size)
    validation_ldg = LatentDataGenerator(encoder,ydf,validation_file_path_list,latent_dim=latent_dim,enc_batch_size=mlp_batch_size)
    ...
    test_dg = DataGenerator(tmp_file_path_list,batch_size=final_batch_size,shuffle=False)
    ...
    if os.path.exists(submission_file):
        ...
        for lst in tqdm(chunks):
            ...
            out = mlp.predict(pred[2])
            for n,index in enumerate(lst):
                y_test[index,:]=out[n,:]

        my_submission = []
        for index,theid in enumerate([x.split('ID_')[-1].split('.')[0] for x in flist]):
        ...
        mysdf.to_csv(submission_file, index=False)

    out=subprocess.check_output(['kaggle','competitions','submit','-c','rsna-intracranial-hemorrhage-detection','-f',submission_file,'-m','init-script'])
    print(out.decode('utf-8'))
